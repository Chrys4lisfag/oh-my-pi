...
Whole-chain restrictions are scanned with deny precedence, even when a matching prompt appears earlier. If the shared shell classifiers identify the resolved shell as cmd or PowerShell (including `pwsh`), the POSIX compound recognizer is not used and approval retains its legacy behavior. The same classifiers select the non-POSIX `/c` and `-Command` execution paths.
