...
All matching whole-chain restrictions are considered, so a later whole-chain deny overrides an earlier prompt. The opt-in is disabled when the resolved shell is `cmd.exe`, whose quoting differs from the POSIX parser; legacy approval behavior applies instead.
