...
- Added opt-in `bash.allowCompoundCommands` approval for conservative literal `&&` chains, with ordered per-segment rules and normal bash policy fallback for unmatched segments. The opt-in is disabled for cmd and PowerShell using the shared shell classifiers, and whole-chain denies take precedence over earlier prompts.
