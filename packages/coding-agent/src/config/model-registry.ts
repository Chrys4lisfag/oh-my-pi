...
export class ModelRegistry {
	...
	async #refreshRuntimeDiscoveries(
		...
		this.#models = this.#withCatalogMetrics(this.#applyRuntimeModelModifiers(this.#unprojectedModels));
		// Notify subscribers once the refreshed model list has settled (covers
		// refresh / refreshProvider / refreshRuntimeProviders). AgentSession uses
		// this to retry a skipped advisor build once late-loading dynamic providers
		// (ollama/litellm/extension catalogs) arrive after startup.
		this.#emitModelsUpdated();
	}
	...
	#configuredDiscoveryCacheProviderId(providerConfig: DiscoveryProviderConfig): string {
		...
		if (providerConfig.discovery.type === "openai-models-list") {
			...
			return providerConfig.discovery.injectV1 === false
				? `${providerConfig.provider}:openai-models-list-bare-context-v3${endpointSuffix}${authSuffix}`
				: `${providerConfig.provider}:openai-models-list-context-v3${endpointSuffix}${authSuffix}`;
		}
		if (providerConfig.discovery.type === "litellm") {
			...
			return `${providerConfig.provider}:litellm-rich-v4${endpointSuffix}${authSuffix}`;
		}
	 ...
	 * rebuilt (see #refreshRuntimeDiscoveries). Returns an unsubscribe function.
	 * Used to retry work that depends on late-loading dynamic providers: the
	 * startup `refreshInBackground()` runs AFTER sessions/advisors are built, so a
	 * configured dynamic model is not yet in the registry at build time.
	 */
