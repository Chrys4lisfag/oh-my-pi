...
export async function discoverOpenAIModelsList(
	...
	// `discovery.baseUrl` aims the catalog fetch at a different upstream than
	// inference, and `injectV1: false` resolves `/models` against that URL
	...
	// model list under a forced `/v1/models`; discovery must match the
	// configured base URL exactly so the full catalog is surfaced.
	const discoverySourceUrl = providerConfig.discovery.baseUrl ?? providerConfig.baseUrl;
	const discoveryBaseUrl = injectV1
		? normalizeOpenAIModelsListDiscoveryBaseUrl(discoverySourceUrl)
		: normalizeBareDiscoveryBaseUrl(discoverySourceUrl);
	...
	const modelsUrl = injectV1
		? buildOpenAIModelsListDiscoveryUrl(discoverySourceUrl)
		: appendModelsPath(discoveryBaseUrl);
