		...
		for (const [name, config] of Object.entries(configs)) {
			...
			const toolsPromise = connectionPromise.then(async connection => {
				try {
					...
					if (mcpTiming) {
						const entry = mcpTiming.get(name);
						if (entry) {
							entry.toolsMs = performance.now() - entry.t0 - (entry.connectMs ?? 0);
							entry.tools = serverTools.length;
						}
					}
					return { connection, serverTools };
				} catch (error) {
					// Detach and delete synchronously, then close in the background:
					// awaiting a slow HTTP close here would keep toolsPromise pending.
					this.#detachConnection(name, connection);
					void disconnectServer(connection).catch(() => {});
					throw error;
				}
