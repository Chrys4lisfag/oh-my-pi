...
export class TurnRecovery {
	...
	isClassifierRefusal(message: AssistantMessage): boolean {
		...
		// Preserve provider safety-filter payloads that omit stopDetails.type.
		return /flagged for possible|content[_ -]policy(?:[_ -]violation)?|content[_ -]filter(?:ed)?|sensitive content|content was blocked|refusal|blocked by (?:google|safety)/i.test(
