...
export class SessionAdvisors {
	...
	rebuildRuntime(seedToCurrent = false): boolean {
		...
		try {
			...
			this.#stopAdvisorRuntime();
			try {
				this.#buildAdvisorRuntime(seedToCurrent, true, previousDescriptors);
	...
	#buildAdvisorRuntime(
		seedToCurrent = false,
		emitWarnings = true,
		resolvedDescriptors?: readonly AdvisorRuntimeDescriptor[],
	): boolean {
		...
		const descriptors = resolvedDescriptors ?? this.#resolveAdvisorRuntimeDescriptors(emitWarnings);
