...
describe("isUsageLimitOutcome", () => {
	...
	// Connect resource_exhausted is commonly a short-lived model-capacity
	// signal, not proof that this credential exhausted its quota. Keep it out
	// of credential rotation so the normal short retry path handles it.
	it("does not rotate on bare Connect resource_exhausted without a usage status", () => {
		expect(isUsageLimitOutcome(undefined, "Connect error resource_exhausted: Error")).toBe(false);
		expect(isUsageLimitOutcome(undefined, "Connect error resource exhausted: Error")).toBe(false);
	});
